...
> The 1€ filter speed coefficient $\beta$ multiplies tracking velocity. In pixel coordinates, fingertip speed often reaches $500\text{--}1500\text{ px/s}$, where $\beta \approx 0.007$ scales the cutoff by $0.007 \times 500 \approx 3.5\text{ Hz}$. In Normalized Device Coordinates $[0, 1]$, fingertip speed is typically $0.1\text{--}2.0\text{ s}^{-1}$. A coefficient of $\beta = 30.0$ scales the dynamic cutoff from $1.0\text{ Hz}$ at rest up to $30\text{--}60\text{ Hz}$ during rapid strokes, providing responsive real-time tracking without phase lag.
...
1. **Tremor Attenuation:** In stationary tracking, `OneEuroFilter` reduces coordinate jitter from $5.90\text{ px}$ to **$1.77\text{ px}$ RMS** at 1080p, while residual noise drops from $6.17\text{ px}$ to **$0.84\text{ px}$** (a **$7.3\times$ residual noise reduction**).
2. **Phase Lag Estimator Resolution:**  
   ...
   Continuous Fourier harmonic phase analysis reveals the true sub-sample phase delay:
   - For $1.5\text{ Hz}$ motion: `RawFilter` and `DeadbandFilter` have $0.1\text{ ms}$ lag, `OneEuroFilter` has $8.0\text{ ms}$ lag, and `EMAFilter` has $9.1\text{ ms}$ lag (triggering a 1-frame whole-sample delay).
3. **Step Settling Dynamics:** `DeadbandFilter` matches the raw sensor response time ($4.2\text{ ms}$ settling time to $\pm 2\%$), while `OneEuroFilter` settles in $37.8\text{ ms}$ and `EMAFilter` requires $54.6\text{ ms}$.
