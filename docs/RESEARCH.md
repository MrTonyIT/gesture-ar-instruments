...
The benchmark suite (`benchmark_filters.py`) feeds five deterministic trajectories at an explicitly synthetic 60 FPS benchmark sampling rate into each filter. All trajectory metadata is explicitly tracked for reproducibility:
...
3. **Step Settling Dynamics:** Discrete frame latency is computed relative to the step occurrence frame (where discrete resolution is bounded by $\Delta t = 16.67\text{ ms}$ at an explicitly synthetic 60 FPS benchmark sampling rate). `RawFilter` and `DeadbandFilter` settle instantaneously at sample 0 ($0.0\text{ ms}$ delay relative to the step event frame). `OneEuroFilter` settles in 2 frames ($33.3\text{ ms}$) and `EMAFilter` settles in 3 frames ($50.0\text{ ms}$).
