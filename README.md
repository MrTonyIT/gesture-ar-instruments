...
<p align="center">
  <img src="https://img.shields.io/badge/Python-3.9%20%7C%203.10%20%7C%203.11%20%7C%203.12%20%7C%203.14-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python Version" />
  ...
  <img src="https://img.shields.io/badge/Tests-50%20Passing-brightgreen?style=for-the-badge&logo=pytest&logoColor=white" alt="Pytest" />
...
All tracking algorithms, geometry primitives, audio synthesis voices, and gesture state transitions are verified by a **50-test headless pytest suite** and an empirical trajectory benchmark suite.
...
- **`one_euro` (Default)**: Casiez et al. (CHI 2012) adaptive cutoff filter ($f_{c,\min} = 1.0\text{ Hz}, \beta = 0.007$). Reduces stationary jitter by **$7.3\times$** while maintaining **$0.0\text{ ms}$ phase lag** during motion.
- **`deadband`**: Dual-threshold velocity-adaptive deadband ($\delta_{\text{static}} = 0.003, \delta_{\text{dynamic}} = 0.012$) with **$4.2\text{ ms}$ step settling time**.
...
All figures below are directly computed from synthetic deterministic test trajectories ($N = 600$ frames at 60 FPS with Gaussian noise $\sigma = 0.003$ NDC) generated by `benchmark_filters.py` and saved to `benchmarks/filter_benchmark_results.csv`:

| Trajectory | Filter | RMSE (NDC) | Jitter RMS (1080p) | Phase Lag (ms) | Settling Time (ms) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Stationary Noisy (3s)** | RawFilter | 0.00307 | **8.719 px** | 0.0 ms | N/A |
| | EMAFilter ($\alpha=0.65$) | 0.00208 | 4.793 px | 0.0 ms | N/A |
| | DeadbandFilter | 0.00271 | 7.029 px | 0.0 ms | N/A |
| | **OneEuroFilter** | **0.00094** | **1.191 px** | 0.0 ms | N/A |
| **Sinusoidal 1.5Hz (4s)** | RawFilter | 0.00202 | 43.151 px | 0.0 ms | N/A |
| | EMAFilter | 0.01211 | 42.289 px | **16.74 ms** | N/A |
| | **DeadbandFilter** | **0.00222** | 43.157 px | **0.00 ms** | N/A |
| | OneEuroFilter | 0.01277 | 44.533 px | **0.00 ms** | N/A |
| **Step Response (2s)** | RawFilter | 0.00107 | 52.980 px | 0.0 ms | 4.2 ms |
| | EMAFilter | 0.01035 | 36.658 px | 0.0 ms | 54.6 ms |
| | **DeadbandFilter** | **0.00087** | 52.673 px | 0.0 ms | **4.2 ms** |
| | OneEuroFilter | 0.00437 | 45.015 px | 0.0 ms | 37.8 ms |

Detailed derivations and analysis are documented in [`docs/RESEARCH.md`](docs/RESEARCH.md).
...
├── requirements.txt             # Pinned project dependencies (including pytest & ruff)
